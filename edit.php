...
class GDGT_Post_Meta_Box extends GDGT_Base {
	...
	public static function delete_post_meta( $post_id ) {
		...
		foreach( static::$all_meta_keys as $meta_key ) {
	...
	public function add_meta_boxes() {
		add_meta_box( static::BASE_ID, static::PLUGIN_NAME, array( &$this, 'product_selector' ), 'post', 'side' );
	}
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( static::BASE_ID . '-js', plugins_url( 'static/js/' . $js_filename, __FILE__ ), array( 'jquery-ui-widget', 'jquery-ui-autocomplete', 'jquery-ui-sortable' ), static::PLUGIN_VERSION, true );
	}
	...
	public function enqueue_styles() {
		wp_enqueue_style( static::BASE_ID . '-css', plugins_url( 'static/css/gdgt-product-selector.css', __FILE__ ), array(), static::PLUGIN_VERSION );
	}
	...
	public function product_selector() {
		...
		if ( current_user_can( get_option( 'gdgt_min_disable_capability', 'edit_posts') ) ) {
			...
			echo '<div class="' . static::BASE_ID . '-disable-module' . '">';
			echo '<input type="checkbox" id="' . $disable_id . '" name="' . $disable_id . '" value="1" ';
			checked( $readonly );
			echo ' /> <label for="' . $disable_id . '">' . __( 'Disable Databox on this post', static::PLUGIN_SLUG ) .  '</label>';
			echo '</div>';
			unset( $disable_id );
		}

		$labels = array(
			'display' => __( 'Display', static::PLUGIN_SLUG ),
			'displayed' => __( 'Displayed', static::PLUGIN_SLUG ),
			'remove' => __( 'Delete', static::PLUGIN_SLUG ),
			'removed' => __( 'Deleted', static::PLUGIN_SLUG ),
			'typeahead' => __( 'Add a product manually:', static::PLUGIN_SLUG ),
			'typeahead_placeholder' => __( 'Start typing...', static::PLUGIN_SLUG ),
			'invalid_key' => __( 'Invalid API key.', static::PLUGIN_SLUG ),
			'no_results' => __( 'No results found.', static::PLUGIN_SLUG )
		);
		...
		if ( $post_id > 0 ) {
			...
			if ( ! empty( $displayed_products ) && is_array( $displayed_products ) ) {
				...
				$products_body .= '<div>';
				$products_body .= '<h4>' . esc_html( $labels['displayed'] ) . '</h4><ol id="' . static::BASE_ID . '-displayed-products">';
			...
			if ( ! empty( $deleted_products ) && is_array( $deleted_products ) ) {
				...
				$products_body .= '<div>';
				$products_body .= '<h4>'. __( 'Deleted', static::PLUGIN_SLUG ) . '</h4><ul id="' . static::BASE_ID . '-deleted-products">';
		...
		echo '<div id="' . static::BASE_ID . '-results">';
		if ( empty( $products_body ) )
			echo '<p id="' . static::BASE_ID . '-results-placeholder">' . esc_html( __( 'No results found.', static::PLUGIN_SLUG ) ) . '</p>';
		else
			echo $products_body;
		echo '</div>';
		unset( $products_body );
	...
	private function clean_product_array( array $products, $allow_instances = false ) {
		...
		foreach( $products as $product ) {
			...
			if ( ! isset( $product['slug'] ) || ! static::is_valid_slug( $product['slug'] ) || ! isset( $product['name'] ) || ( $allow_instances === false && isset( $product['parent'] ) ) )
				continue;
			...
			if ( isset( $product['parent'] ) ) {
				...
				if ( empty( $product['parent'] ) || ! static::is_valid_slug( $product['parent'] ) || in_array( $product['parent'], $this->processed_product_slugs ) )
					continue;
	...
	public function process_saved_data( $post_id ) {
		...
		if ( ! array_key_exists( static::NONCE, $_POST ) || ! wp_verify_nonce( $_POST[static::NONCE], plugin_basename(__FILE__) ) )
			return;
		...
		if ( ! empty( $hidden_post_boxes ) && is_array( $hidden_post_boxes ) && in_array( static::BASE_ID, $hidden_post_boxes, true ) ) {
			return;
		}
